...
export function day4part2(boards: number[][], calls: number[]) {
	const finishedBoards: number[] = [];
	for (const call of calls) {
		for (let boardIndex = 0; boardIndex < boards.length; boardIndex++) {
			if (!finishedBoards.includes(boardIndex)) {
				...
				for (let rowIndex = 0; rowIndex < 5; rowIndex++) {
					...
					if (isEqual(row, winningValue) || isEqual(column, winningValue)) {
						if (finishedBoards.length < boards.length - 1) {
							finishedBoards.push(boardIndex);
							break;
						} else {
